...
resource "kubectl_manifest" "mlflow_database_dynamic_pvc" {
  depends_on = [kubernetes_namespace.mlflow]
  yaml_body = file("${path.module}/manifests/dynamic_pvc.yaml")
}

resource "kubectl_manifest" "mlflow_database_deployment" {
  depends_on = [
    kubernetes_namespace.mlflow,
    kubectl_manifest.mlflow_database_dynamic_pvc
  ]
